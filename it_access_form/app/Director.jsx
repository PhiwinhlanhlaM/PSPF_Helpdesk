// Screen 4 — Director review & sign-off (access already provisioned by IT officer)
  ...
  return (
    <div className="page slide-up">
      <div className="page-header">
        <div>
          <h1 className="page-title">Director review</h1>
          <p className="page-subtitle">Access has been provisioned by IT. Your signature confirms and records the authorisation.</p>
        </div>
      </div>

      <h2 className="section-title" style={{ marginBottom: 12 }}>Awaiting your sign-off · {pending.length}</h2>
      {pending.length === 0 ? (
      ...
      <div className="dir-card-foot">
        <div className="row gap-1">
          {chainSteps(request).slice(0, 3).map((s, i) => (
            <div key={i} className="chain-dot done"><Icon name="check" size={10} stroke={2.8}/></div>
          ))}
          <div className="chain-dot pending"><Icon name="clock" size={10}/></div>
        </div>
...
function DirectorSign({ requestId }) {
  ...
  async function approve() {
    ...
    try {
      ...
      toast({ title: "Sign-off recorded", body: `${request.employee.name}'s access has been authorised.` });
    } catch (err) {
    ...
    <div className="page slide-up">
      ...
      <div className="page-header" style={{ marginTop: 12 }}>
        <div>
          ...
          <p className="page-subtitle">Access has already been provisioned by the IT officer. Your signature serves as the authorisation record.</p>
        </div>
      </div>
        ...
        <section className="card card-pad">
          ...
          <div className="dir-summary-grid">
            ...
            <div><span className="muted" style={{ fontSize: 11.5, textTransform: "uppercase", letterSpacing: 0.06 }}>Start date</span>
              ...
              <span className="muted" style={{ fontSize: 12 }}>requested by {getPerson(request.submittedBy)?.name}</span></div>
          </div>

          <span className="section-title" style={{ marginTop: 20 }}>Approved systems</span>
          ...
          <span className="section-title" style={{ marginTop: 20 }}>Approval chain</span>
        ...
        <aside className="col gap-4" style={{ position: "sticky", top: 84, alignSelf: "start" }}>
          {stage === "review" && (
            <section className="card card-pad">
              ...
              <p className="card-subtitle">Sign to confirm the provisioned access is authorised, or flag an issue if something was provisioned incorrectly.</p>
              <button className="btn btn-success btn-lg" style={{ width: "100%", justifyContent: "center" }} onClick={() => setStage("sign")}>
                <Icon name="shield-check" size={16}/> Sign &amp; authorise
              </button>
          ...
          {stage === "sign" || stage === "submitting" ? (
            <section className="card card-pad slide-up">
              ...
              <p className="card-subtitle">Your signature records that you have reviewed and authorised this provisioned access.</p>
              <SignaturePad onChange={setSignature}/>
              <label className="confirm-row">
                <input type="checkbox" checked={confirmed} onChange={e => setConfirmed(e.target.checked)}/>
                <span>I confirm I have reviewed this request and authorise the access that has been provisioned.</span>
              </label>
              <div className="row gap-2" style={{ justifyContent: "flex-end", marginTop: 16 }}>
                ...
                <button className="btn btn-primary" disabled={!signature || !confirmed || stage === "submitting"} onClick={approve}>
          ...
          {stage === "rejecting" && (
            <section className="card card-pad slide-up">
              <h2 className="card-title" style={{ color: "var(--red-700)" }}>Flag an issue</h2>
              <Field label="Reason" required help="Minimum 10 characters">
                <textarea className="textarea" rows={5} value={rejectReason} onChange={e => setRejectReason(e.target.value)}
                  placeholder="e.g. Access scope exceeds policy for this role grade…"/>
              </Field>
              <div className="row gap-2" style={{ justifyContent: "flex-end", marginTop: 16 }}>
...
function OfficerPerformance() {
  ...
  const officers = useMemo(() => {
    ...
    state.requests.forEach(r => {
      r.approvals.forEach(a => {
        if (a.role !== "officer-1" && a.role !== "officer-2") return;
        const key = a.personId;
        if (!map[key]) {
          map[key] = {
            id:       a.personId,
            name:     a.approverName || ("Officer #" + a.personId),
            approved: 0,
            rejected: 0,
            totalMs:  0,
            countMs:  0,
        ...
        if (a.action === "approved") map[key].approved++;
        if (a.action === "rejected") map[key].rejected++;
        // turnaround = time from submission to this officer's action
        if (a.action === "approved" && r.submittedAt && a.at) {
          const ms = new Date(a.at) - new Date(r.submittedAt);
          if (ms > 0) { map[key].totalMs += ms; map[key].countMs++; }
        }
      });
    });
    ...
    return Object.values(map)
      .map(o => {
        ...
        const avgMs  = o.countMs > 0 ? o.totalMs / o.countMs : null;
        let avgLabel = "—";
        if (avgMs !== null) {
          const h = avgMs / 3600000;
          if (h < 1)       avgLabel = Math.round(h * 60) + "m";
          else if (h < 24) avgLabel = h.toFixed(1) + "h";
          else             avgLabel = (h / 24).toFixed(1) + "d";
        }
        return { ...o, total, avgLabel };
      })
  ...
  return (
    <div style={{ marginTop: 32 }}>
      <h2 className="section-title" style={{ marginBottom: 12 }}>Officer performance</h2>
      <div className="card" style={{ overflow: "hidden" }}>
        <table className="table">
          <thead>
            <tr>
              ...
              <th style={{ textAlign: "center" }}>Avg. review time</th>
              <th style={{ textAlign: "right" }}>Approval rate</th>
            </tr>
          </thead>
          <tbody>
            {officers.map(o => {
              const rate = o.total > 0 ? Math.round((o.approved / o.total) * 100) : null;
              const barColor = rate === null ? "var(--ink-200)" : rate >= 80 ? "var(--green-500)" : rate >= 50 ? "var(--amber-500)" : "var(--red-500)";
              return (
                <tr key={o.id}>
                  <td>
                    <div className="row gap-2" style={{ alignItems: "center" }}>
                      <div style={{
                        ...
                        background: "var(--pspf-100)", color: "var(--pspf-800)",
                        display: "flex", alignItems: "center", justifyContent: "center",
                        fontSize: 11, fontWeight: 700, flexShrink: 0,
                      }}>{initials(o.name)}</div>
                      <span style={{ fontWeight: 550 }}>{o.name}</span>
                    </div>
                  </td>
                  <td style={{ textAlign: "center" }}>
                    <span style={{ color: "var(--green-700)", fontWeight: 600 }}>{o.approved}</span>
                  </td>
                  <td style={{ textAlign: "center" }}>
                    <span style={{ color: o.rejected > 0 ? "var(--red-600)" : "var(--ink-400)", fontWeight: o.rejected > 0 ? 600 : 400 }}>{o.rejected}</span>
                  </td>
                  <td style={{ textAlign: "center", color: "var(--ink-700)" }}>{o.total}</td>
                  <td style={{ textAlign: "center" }}>
                    <span className="badge badge-gray">{o.avgLabel}</span>
                  </td>
                  <td style={{ textAlign: "right", minWidth: 120 }}>
                    {rate !== null ? (
                      <div className="row gap-2" style={{ alignItems: "center", justifyContent: "flex-end" }}>
                        <div style={{ flex: 1, maxWidth: 80, height: 6, background: "var(--ink-100)", borderRadius: 99, overflow: "hidden" }}>
