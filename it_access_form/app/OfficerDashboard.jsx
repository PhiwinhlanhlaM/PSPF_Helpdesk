...
function ClaimModal({ request, onConfirm, onCancel }) {
  const [selected, setSelected] = useState(() =>
    request.systems.map(s => s.id) // default: all selected
  );
  ...
  function selectAll() { setSelected(request.systems.map(s => s.id)); }
    ...
    <div style={{
      ...
      <div className="card card-pad slide-up" style={{ width: 480, maxWidth: "95vw", maxHeight: "85vh", overflowY: "auto" }}>
        ...
        <p className="card-subtitle">
          By claiming, you confirm you can provision the selected systems.
...
function OfficerDashboard() {
  ...
  const counts = useMemo(() => {
    ...
    const provisioned = reqs.filter(r => r.status === "provisioned" && r.submittedAt && r.provisionedAt);
    let avgTurnaround = "—";
    if (provisioned.length > 0) {
      const totalMs = provisioned.reduce((sum, r) => {
        return sum + (new Date(r.provisionedAt) - new Date(r.submittedAt));
      }, 0);
      const avgMs = totalMs / provisioned.length;
      const avgH  = avgMs / 3600000;
      if (avgH < 1)        avgTurnaround = Math.round(avgH * 60) + "m";
      else if (avgH < 24)  avgTurnaround = avgH.toFixed(1) + "h";
      else                 avgTurnaround = (avgH / 24).toFixed(1) + "d";
    }
    return {
      new: reqs.filter(r => r.status === "new").length,
      pending: reqs.filter(r => ["claimed", "awaiting-officer-2"].includes(r.status) && (r.claimedBy === me.id || r.status === "awaiting-officer-2")).length,
      all: reqs.filter(r => r.status !== "provisioned").length + reqs.filter(r => r.status === "provisioned").length,
      avgTurnaround,
    };
  ...
  const visibleRequests = useMemo(() => {
    ...
    else if (filter === "pending") list = list.filter(r => ["claimed", "awaiting-officer-2"].includes(r.status) && (r.claimedBy === me.id || r.status === "awaiting-officer-2"));
    if (search.trim()) {
  ...
  function confirmClaim(request, actionedSystems) {
    ...
    // Navigate straight to sign screen
    ...
    <div className="page slide-up">
      ...
      <div className="page-header">
        <div>
          ...
          <p className="page-subtitle">Claim requests, provision access, and sign off to confirm it's been granted.</p>
        </div>
      ...
      <div className="dash-stats">
        ...
        <StatCard label="Avg. turnaround" value={counts.avgTurnaround} kind="gray" subtle/>
      </div>
        ...
        <div className="card" style={{ overflow: "hidden" }}>
          <div className="dash-tabs">
            ...
            <button className={"dash-tab " + (filter === "all" ? "active" : "")} onClick={() => setFilter("all")}>
              All <span className="tab-count">{state.requests.length}</span>
            </button>
          </div>
              ...
              <table className="table">
                ...
                <tbody>
                  {visibleRequests.map(r => {
                    ...
                    const isMine = r.claimedBy === me.id;
                    const next = nextStep(r);
                    const canClaim = r.status === "new" && !r.claimedBy;
                    const canSign = isMine && (next === "officer-1" || next === "officer-2");
                    const submitter = getPerson(r.submittedBy);
                    return (
                      <tr key={r.id}
                        ...
                        <td>
                          <div className="col">
                            ...
                            <span className="muted" style={{ fontSize: 11.5 }}>by {submitter?.name}</span>
                          </div>
                        </td>
                          ...
                          <div className="row gap-2" style={{ justifyContent: "flex-end" }} onClick={e => e.stopPropagation()}>
                            ...
                            {canSign && (
                              <button className="btn btn-primary btn-sm" onClick={() => gotoSign(r.id)}>
                                Approve <Icon name="chevron-right" size={12}/>
                              </button>
                            )}
                            {!canClaim && !canSign && (
...
function DetailPanel({ request, onClose, onClaim, onSign }) {
  ...
  const submitter = getPerson(request.submittedBy);
  const next = nextStep(request);
  const canClaim = request.status === "new" && !request.claimedBy;
  const canSign = request.claimedBy === me.id && (next === "officer-1" || next === "officer-2");
      ...
      <div className="detail-body">
        <section>
          ...
          <div className="kv-grid">
            ...
            <span>Submitted by</span><span>{submitter?.name}</span>
          </div>
        </section>
      ...
      <div className="detail-actions">
        ...
        {canSign && (
          <button className="btn btn-primary" style={{ flex: 1 }} onClick={onSign}>
            Approve & sign <Icon name="chevron-right" size={14}/>
          </button>
        )}
        {!canClaim && !canSign && (
          <span className="muted" style={{ fontSize: 12.5, textAlign: "center", flex: 1 }}>
            {request.status === "rejected" ? "This request was rejected." :
             request.status === "provisioned" ? "Access has been provisioned." :
             request.status === "awaiting-director" ? "Access provisioned — awaiting director sign-off." :
        ...
        return (
          <div key={step.key} className={"timeline-step " + dotCls}>
            ...
            <div className="col" style={{ flex: 1, minWidth: 0 }}>
              ...
              <span className="muted" style={{ fontSize: 12 }}>
                {step.person?.name === "You" ? "Alex Morgan" : step.person?.name}
                {a && a.action === "approved" && " · approved"}
