...
if (!$request_id) header("Location: supervisor_dashboard.php");
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($action == 'approve') {
        $conn->prepare("
            ...
            SET supervisor_id=?, status='pending_hrm', updated_at=NOW()
            WHERE request_id=?
        ")->execute([$_SESSION['user_id'], $request_id]);

        $conn->prepare("
            INSERT INTO request_logs (request_id, action_by, action, created_at)
            VALUES (?, ?, 'Supervisor approved request', NOW())
        ")->execute([$request_id, $_SESSION['user_id']]);

        sendRequestEmail($conn, $request_id, 'supervisor_approved');
    } else {
        ...
        $conn->prepare("
            INSERT INTO request_logs (request_id, action_by, action, created_at)
            VALUES (?, ?, 'Supervisor rejected request', NOW())
        ")->execute([$request_id, $_SESSION['user_id']]);

        sendRequestEmail($conn, $request_id, 'supervisor_rejected');
    }
