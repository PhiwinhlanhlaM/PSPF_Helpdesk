...
// include helpers (paths assume this file is in project root and includes/ is sibling)
require_once '../includes/auth_helpers.php';
require_once '../includes/role_switcher.php';
require_once '../db.php'; // ensure $conn available
...
$params = [$UserUsername, $searchLike, $searchLike, $searchLike];
$types  = "ssss";
...
function statusBadge(string $s): string {
    return match (strtolower($s)) {
        ...
        'pending feedback' => 'badge bg-info',
...
function statusColor(string $status): string {
    return match (strtolower($status)) {
        ...
        'pending feedback' => 'status-pending-feedback',
...
<head>
  <meta charset="UTF-8">
  <title>Your tickets - PSPF CRM</title>
...
<main id="main-content">
 ...
 <div class="container mt-5">
  <div class="settings-header">   
        <h1 class="settings-title">My tickets</h1>
        ...
        <div class="card-body">
            <form method="GET" class="row gy-2 gx-3 mb-4" onsubmit="return validateSearchForm()">
                ...
                <select name="status" class="form-select form-select-sm">
                    ...
                    <option value="pending feedback" <?= $statusFilter == 'pending feedback' ? 'selected' : '' ?>>Pending Feedback</option>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <span class="status-button <?= htmlspecialchars(statusColor($ticket['status'])) ?>">
                                            ...
                                            <?php 
                                            $displayStatus = match(strtolower($ticket['status'])) {
                                    ...
                                    <td>
                                        ...
                                        if (strtolower($ticket['status']) === 'resolved'):
                                            // Fetch feedback for resolved tickets
                                            $feedbackSql = "SELECT rating, comment, created_at 
                                                        FROM ticket_feedback 
                                                        WHERE ticket_id = ? 
                                                        ORDER BY created_at DESC LIMIT 1";
                                            $feedbackStmt = $conn->prepare($feedbackSql);
                                            $feedbackStmt->bind_param('i', $ticket['id']);
                                        ...
                                        ?>
                                                <div class="status-reason-badge mt-1" 
                                                    title="Feedback submitted on <?= formatDateTime($feedback['created_at']) ?>">
                                                    <i class="bi bi-star-fill text-warning me-1"></i>
                                                    <small class="text-muted">
                                                        Rating: <?= str_repeat('⭐', $feedback['rating']) ?>
                                                        <?php if (!empty($feedback['comment'])): ?>
                                                            <br>
                                                            <span class="fw-light">"<?= htmlspecialchars(truncateText($feedback['comment'], 50)) ?>"</span>
                                                        <?php endif; ?>
                                                    </small>
                                                </div>
                                        <?php 
                                        ...
                                        elseif (strtolower($ticket['status']) === 'pending feedback'):
                                            // For pending feedback, show that feedback is awaited
                                        ?>
                                            <div class="status-reason-badge mt-1 pending-feedback">
                                                <i class="bi bi-hourglass-split text-info me-1"></i>
                                                <small class="text-info">
                                                    <i class="bi bi-chat-dots"></i> Awaiting your feedback
                                                </small>
                                            </div>
                                        <?php
                                        else:
                                            // For other statuses, show latest status reason from logs
                                            $reasonSql = "SELECT change_reason, new_status, change_date 
                                                        FROM ticket_status_logs 
                                                        WHERE ticket_id = ? AND change_reason IS NOT NULL AND change_reason != ''
                                                        ORDER BY change_date DESC LIMIT 1";
                                            $reasonStmt = $conn->prepare($reasonSql);
                                            $reasonStmt->bind_param('i', $ticket['id']);
                                            $reasonStmt->execute();
                                            $reasonResult = $reasonStmt->get_result();
                                            if ($reasonResult->num_rows > 0):
                                                $reasonData = $reasonResult->fetch_assoc();
                                        ?>
                                                <div class="status-reason-badge mt-1" 
                                                    title="Status changed to <?= htmlspecialchars($reasonData['new_status']) ?> on <?= formatDateTime($reasonData['change_date']) ?>">
                                                    <i class="bi bi-info-circle text-muted me-1"></i>
                                                    <small class="text-muted">
                                                        <?= htmlspecialchars(truncateText($reasonData['change_reason'], 50)) ?>
                                                    </small>
                                                </div>
                                        <?php 
                                            endif;
    ...
    <div class="modal-content shadow-lg border-0">
      ...
      <div class="modal-body bg-light">
        <div class="container-fluid">
          ...
          <!-- Feedback Section - Only shown for resolved tickets -->
            <div id="feedbackSection" class="mt-3" style="display: none;">
                <div class="card border-0 shadow-sm bg-light">
                    <div class="card-body">
                        <h6 class="fw-semibold mb-2">
                            <i class="bi bi-star-fill text-warning me-1"></i> Your Feedback
                        </h6>
                        <div id="feedbackDisplay" class="p-3 bg-white rounded border">
                            <div id="feedbackRating" class="mb-2"></div>
                            <div id="feedbackComment" class="text-dark"></div>
                            <small id="feedbackDate" class="text-muted d-block mt-2"></small>
                        </div>
                    </div>
                </div>
            </div>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.querySelectorAll('.view-ticket-btn').forEach(button => {
        button.addEventListener('click', async function() {
            ...
            try {
                const response = await fetch(`./get_ticket_details_ajax.php?ticket_id=${ticketId}`);
                ...
                if (data.success) {
                    ...
                    // Inside your view ticket click handler, after setting other fields
                    if (data.ticket.feedback) {
                        document.getElementById('feedbackSection').style.display = 'block';
                        
                        // Display rating as stars
                        const rating = data.ticket.feedback.rating;
                        const stars = '⭐'.repeat(rating);
                        document.getElementById('feedbackRating').innerHTML = `<strong>Rating:</strong> ${stars} (${rating}/5)`;
                        
                        // Display comment
                        document.getElementById('feedbackComment').innerHTML = data.ticket.feedback.comment || '<em>No comment provided</em>';
                        
                        // Display date
                        document.getElementById('feedbackDate').textContent = `Submitted on: ${data.ticket.feedback.created_at}`;
                    } else {
    ...
    // Notifications functionality (if you have this)
