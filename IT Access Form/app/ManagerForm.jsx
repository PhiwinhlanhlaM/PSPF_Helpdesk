...
function ManagerForm() {
  ...
  // Persist the typed full name to the user's account. Returns the saved name,
  // or null on failure. Once saved, future requests show it in "Submitted by".
  async function saveFullName() {
    const name = fullNameInput.trim().replace(/\s+/g, " ");
    if (name.length < 2) {
    ...
    try {
      ...
      const res = await fetch("/pspf_crm/api/it_access/profile_name.php", {
        method: "POST",
        credentials: "include",
        headers: { "Content-Type": "application/json", "X-CSRF-Token": csrf },
        body: JSON.stringify({ full_name: name }),
      });
      if (!res.ok) { const e = await res.json().catch(() => ({})); throw new Error(e.error || "Could not save name"); }
      const data = await res.json();
      const saved = data.full_name || name;
      onNameSaved(saved); // updates me.name + clears needName app-wide
      return saved;
    } catch (err) {
      setErrors(e => ({ ...e, fullName: err.message }));
      return null;
    }
  }
  ...
  function submit(ev) {
    ...
    if (!managerSignature) {
      toast({ kind: "error", title: "Signature required", body: "Please sign the request before submitting." });
      document.getElementById("manager-sign-card")?.scrollIntoView?.({ behavior: "smooth", block: "center" });
      return;
    }
    setIsSending(true);
    (async () => {
      // First-time submitters must save their full name (captured in "Submitted by").
      if (needName) {
        const saved = await saveFullName();
        if (!saved) { setIsSending(false); return; }
      }
      try {
        ...
        <div className="form-main">
          ...
          <section className="card card-pad">
            ...
            <div className="grid-2">
              ...
              <div style={{ gridColumn: "span 2" }}>
                {needName ? (
                  <Field label="Submitted by (enter your full name)" required error={errors.fullName}>
                    <input
                      className={"input" + (errors.fullName ? " input-error" : "")}
                      value={fullNameInput}
                      onChange={e => { setFullNameInput(e.target.value); if (errors.fullName) setErrors(er => ({ ...er, fullName: null })); }}
                    />
                  </Field>
                ) : (
          ...
          <section className="card card-pad">
            ...
            <p className="card-subtitle">
              {form.requestType === "change"
                ? "Explain why the access change is required."
            ...
            <Field error={errors.justification}>
              ...
              <div className="row" style={{ justifyContent: "space-between", marginTop: 4 }}>
                <span className="help">Visible to all actioners in the chain</span>
          ...
          <section className="card card-pad sign-panel" id="manager-sign-card">
            ...
            <label className="confirm-row">
              <input type="checkbox" checked={confirmedAuth} onChange={e => setConfirmedAuth(e.target.checked)}/>
    ...
    <div className="page slide-up">
      <div className="success-card scale-in">
        ...
        <p className="page-subtitle" style={{ textAlign: "center", marginBottom: 20 }}>
          Your request has been routed to the IT team for action, then to the Director for sign-off.
        </p>
        ...
        <div className="success-chain">
          <div className="chain-step done">
            ...
            <div className="col"><strong>Admin action</strong><span className="muted">Submitted just now</span></div>
          </div>
          <div className="chain-line"/>
          <div className="chain-step pending">
            <div className="chain-dot pending"><Icon name="clock" size={11}/></div>
            <div className="col"><strong>IT Officer action</strong><span className="muted">Up next · typically within 4 hours</span></div>
          </div>
          <div className="chain-line"/>
          <div className="chain-step">
            <div className="chain-dot"><span>3</span></div>
            <div className="col"><strong>Director sign-off</strong><span className="muted">Final action</span></div>
          </div>
        </div>
