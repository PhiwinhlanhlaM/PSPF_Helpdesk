...
function sendRequestEmail($conn, $request_id, $stage) {
    ...
    $driver = $conn->query("SELECT email FROM users WHERE role = 'driver' AND active = 1 LIMIT 1")
                   ->fetch(PDO::FETCH_ASSOC);

    // HRM (first active HRM found)
    $hrm = $conn->query("SELECT email FROM users WHERE role = 'hrm' AND active = 1 LIMIT 1")
                 ->fetch(PDO::FETCH_ASSOC);
